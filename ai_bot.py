...
def _sync_generate_comment(post_text: str) -> str | None:
    ...
    if not client:
        return None
    prompt = (
        "أنت خبير في علم الإنسان (الأنثروبولوجيا) ومعلم شغوف تتميز بوضع طابع اسلامي بحت على منشورات علم الانثروبولوجيا. "
        "قناة تيليغرام أنثروبولوجية نشرت للتو هذا المنشور:\n\n"
        f"{post_text[:1500]}\n\n"
        "المطلوب: قم بإقتباس معلومات اسلامية عن علوم الانثروبولوجيا اكتب تعليقاً علمياً قصيراً (90 إلى 200 كلمة) يُضيف:\n"
        "- معلومة تكميلية مفيدة أو منظور أنثروبولوجي مختلف يكون بالطريقة الإسلامية اجعل كل منشوراتك تكون على حضارة وآثار الجزائر\n"
...
def _sync_translate_article(title: str, summary: str) -> str | None:
    ...
    if not client:
        return None
    prompt = (
        "أنت خبير في علم الإنسان (الأنثروبولوجيا) وكاتب عربي متمكن اجعل من هلم الانثروبولوجيا يكون كأنو طابع اسلامي بحت. "
        ...
        "- أضف أجعل من كلماتك تكون ءوغ طابع اسلامي بحت سياقاً أنثروبولوجياً يربط الموضوع بالإنسانية الكبرى\n"
...
async def scrape_and_publish(context: ContextTypes.DEFAULT_TYPE) -> None:
    """تُشغَّل كل ساعتين — تسحب مقالاً جديداً وتنشره في القناة."""
...
async def handle_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        print("[start_handler] ✅ رد على /start")
    except Exception as e:
        print(f"[start_handler] ❌ {e}")
        try:
            await update.message.reply_text(START_TEXT)
        except Exception as e2:
            print(f"[start_handler] ❌ فشل كل المحاولات: {e2}")
...
async def handle_private_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        msg = update.message
        if not msg:
            print("[private_handler] ⚠️ update.message = None")
            return
        if not msg.text:
        ...
        print(f"[private_handler] 💬 سؤال من {msg.chat_id}: {user_text[:80]}")
        ...
        try:
            ...
            print("[private_handler] ⏱️ Groq تجاوز 60 ثانية")
            answer = "⚠️ استغرقت الإجابة وقتاً طويلاً، حاول مجدداً."
        except Exception as e:
            print(f"[private_handler] ❌ خطأ Groq: {e}")
            answer = "❌ تعذّر الحصول على إجابة، حاول مرة أخرى."

        print(f"[private_handler] 📝 الجواب ({len(answer)} حرف): {answer[:60]}...")

        try:
            await msg.reply_text(answer)
            print(f"[private_handler] ✅ أُرسل الرد بنجاح")
        except Exception as e:
            print(f"[private_handler] ❌ فشل reply_text: {e}")
            await msg.reply_text(answer[:4000])

    except Exception as fatal:
        print(f"[private_handler] 💥 خطأ غير متوقع: {fatal}")
...
async def handle_group_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        print(
            f"[group_handler] 📥 chat={msg.chat_id} msg={msg.message_id} "
            f"auto={is_auto} sender={getattr(sender,'username','—')} "
